...
# The entrypoint only runs at container start. Shells created
# by "podman exec" inherit the container's base environment,
# which lacks vars exported above. Writing them to a file
# sourced by ~/.zshenv (all zsh) and /etc/profile.d (bash -l).
# ============================================================
_ENTRYPOINT_ENV="/run/entrypoint-env.sh"
...
# Symlink into profile.d for bash login shells
ln -sf "$_ENTRYPOINT_ENV" /etc/profile.d/99-entrypoint-env.sh
