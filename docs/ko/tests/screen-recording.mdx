...
description: >-
  Xvfb, headed Chromium(Playwright 사용), ffmpeg 화면 녹화가 devcontainer 내부에서 모두 함께
  작동하는지 증명합니다.
sidebar:
  order: 1
i18n:
...
이 테스트는 headed 브라우저 세션을 MP4 파일로 녹화하여 전체 VNC/브라우저 스택을 검증합니다. 다음 사항을 증명합니다:

- **Xvfb**가 실행 중이며 `:99`에서 연결을 수락하고 있음
- **Chromium**이 가상 디스플레이에서 headed 모드로 실행될 수 있음
...
3. Playwright의 `sync_api`를 통해 headed Chromium을 실행합니다
4. `https://example.com`으로 이동(3초 대기)한 후 `https://httpbin.org`로 이동합니다(3초 대기)
5. 깨끗한 MP4 완료를 위해 ffmpeg에 SIGTERM을 전송합니다
