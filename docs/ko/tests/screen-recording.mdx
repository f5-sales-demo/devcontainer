...
title: 화면 녹화 테스트
description: >-
  devcontainer 내부에서 Xvfb, 헤디드 Chromium(Playwright 경유), ffmpeg 화면 녹화가 모두 함께 작동하는지
  증명합니다.
sidebar:
  order: 1
i18n:
...
이 테스트는 헤디드 브라우저 세션을 MP4 파일로 녹화하여 전체 VNC/브라우저 스택을 검증합니다. 다음 사항을 증명합니다:

- **Xvfb**가 실행 중이며 `:99`에서 연결을 수락하고 있음
- **Chromium**이 가상 디스플레이에서 헤디드 모드로 실행될 수 있음
- **ffmpeg**가 x11grab을 통해 디스플레이를 캡처할 수 있음
- 결과 **MP4**가 유효하고 재생 가능한 비디오임

## 작동 방식

1. X 디스플레이가 준비될 때까지 `xdpyinfo -display :99`를 폴링합니다 (엔트리포인트와 동일한 패턴)
2. 10 fps로 디스플레이를 녹화하는 ffmpeg를 시작합니다
3. Playwright의 `sync_api`를 통해 헤디드 Chromium을 실행합니다
4. `https://example.com`으로 이동 (3초 대기), 그 다음 `https://httpbin.org`로 이동 (3초 대기)
5. 깔끔한 MP4 마무리를 위해 ffmpeg에 SIGTERM을 전송합니다
6. 출력 경로와 파일 크기를 보고합니다

## 스크립트

다음 내용을 컨테이너 내부에 `vnc-browser-test.py`로 저장하거나, 이 저장소의 `docs/tests/` 디렉토리에서 복사하세요:
...
| `ffmpeg: x11grab error` | 디스플레이 불일치 또는 ffmpeg에 x11grab 지원이 누락됨 |
| `Playwright launch error` | Chromium이 설치되지 않았거나 `--no-sandbox`가 누락됨 |
| MP4가 0바이트 | ffmpeg가 쓰기 전에 종료됨 — SIGTERM 처리를 확인하세요 |
