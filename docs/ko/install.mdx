...
4. 현재 디렉토리에 `docker-compose.yml`, `devcontainer.sh`, `.env` 템플릿 다운로드

:::note[Homebrew 필요]
Homebrew가 설치되어 있지 않으면 스크립트가 안내 메시지와 함께 종료됩니다.
IT 부서에서 Homebrew를 관리형 소프트웨어로 제공할 수 있으니 먼저 확인하세요.
:::
...
설치 프로그램을 실행하면 디렉토리에 다음 파일이 생성됩니다:
