...
이 페이지는 저장소를 클론하거나, 이미지를 로컬에서 빌드하거나, Dockerfile을 커스터마이즈하려는 개발자를 위한 것입니다. 사전 빌드된 컨테이너를 사용하려면 [시작하기](./getting-started)를 참조하세요.
...
`docker-compose.build.yml` 파일은 로컬 빌드 지원을 추가합니다. 이 파일은 자동으로 병합되지 **않으므로** `-f` 플래그를 사용하여 명시적으로 전달해야 합니다. 이는 저장소를 클론했든 아니든 `docker compose up -d` (또는 `podman-compose up -d`)가 항상 GHCR에서 사전 빌드된 이미지를 가져온다는 것을 의미합니다.
...
이 명령은 빌드 파일을 기본 compose 파일과 병합하고, 로컬 `Dockerfile`에서 이미지를 빌드한 후 컨테이너를 시작합니다. `--build` 플래그는 캐시된 이미지가 있더라도 강제로 재빌드합니다.
...
| 2b. 보안 APT 패키지 | 네트워크 분석, 웹 스캐너, 패스워드 도구, 리버스 엔지니어링, 포렌식 (~80개 패키지) |
| 3. Python 부트스트랩 | 심볼릭 링크 + get-pip.py를 통한 pip |
| 4. Go | 공식 tarball (빌드 시점에 해석되는 최신 안정 버전) |
| 5. Rust | rustup (시스템 전체, 최신 안정 버전) |
| 6. Maven + Gradle | `/opt`에 바이너리 다운로드 |
| 7. VNC 스택 | Xvfb, x11vnc, noVNC, fluxbox — 브라우저를 통한 원격 디스플레이 |
| 8. Nerd Fonts | JetBrainsMono, Hack, FiraCode (최신 릴리스) |

### 2단계: `final` (변동이 잦은 도구 + 사용자 설정, ~1.5 GB)

이 레이어들은 더 자주 변경되지만(npm/pip 업데이트, 설정 변경), 무거운 `deps` 단계가 캐시되므로 빠르게 재빌드됩니다.
...
| 10j. Metasploit | 익스플로이테이션 프레임워크 (amd64 전용) |
...
| 13. Playwright 브라우저 | `playwright install --with-deps`를 통한 Chromium + 시스템 의존성 |
| 13b. Chrome DevTools MCP | Chrome 심볼릭 링크 + MCP 사전 캐시 + 헤드리스 패치 |
| 14. Homebrew | Linuxbrew (AI 어시스턴트 의존성 + 포맷터) |
| 15. ZSH 플러그인 | oh-my-zsh 커스텀 플러그인 |
| 16. 셸 부트스트랩 | npm-global, tfenv, compinit |
| 17. Claude Code + Codex 설정 | 도구 인식 메모리, 관리 정책, 자체 테스트 스크립트, Codex 설정 |
| 18. 엔트리포인트 | 컨테이너 시작 스크립트 (절대적으로 마지막 COPY) |

## 도구 추가

`Dockerfile`을 편집하고 적절한 섹션에 도구를 추가합니다. 추가 후 재빌드하세요:
...
섹션 2의 `apt-get install` 블록에 추가합니다:
...
섹션 11의 `npm install -g` 블록에 추가합니다:
...
섹션 10에 추가하거나 아키텍처 감지가 포함된 새 `RUN` 레이어를 생성합니다:
...
CI는 **레지스트리 기반 캐싱**을 사용합니다 — 빌드 레이어는 GitHub Actions 캐시 대신 GHCR(`ghcr.io/f5xc-salesdemos/devcontainer:cache-*`)에 저장됩니다. 이를 통해 이 ~6 GB 멀티 아키텍처 이미지에서 잦은 전체 재빌드를 유발하는 10 GB GHA 캐시 제한을 방지합니다.

2단계 아키텍처는 레지스트리 캐시와 함께 다음과 같이 작동합니다:

- **도구 버전 업데이트** (npm, pip, 바이너리 도구) — `final` 단계만 재빌드 (~5분)
- **설정 파일 변경** (`claude-config/`, `entrypoint.sh`) — 마지막 몇 개 레이어만 재빌드 (~1분)
- **기반 변경** (APT 패키지, Go/Rust/Java 버전) — 양 단계 전체 재빌드 (~30분)
