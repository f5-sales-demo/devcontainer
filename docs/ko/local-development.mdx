---
title: 로컬 개발
description: '리포지토리를 클론하고, 로컬에서 빌드하며, Docker 이미지를 커스터마이즈합니다.'
sidebar:
  order: 5
i18n:
...
이 페이지는 리포지토리를 클론하거나, 로컬에서 이미지를 빌드하거나, Dockerfile을 커스터마이즈하려는 개발자를 위한 것입니다. 사전 빌드된 컨테이너를 사용하기만 원한다면 [시작하기](./getting-started)를 참조하세요.
...
`docker-compose.build.yml` 파일은 로컬 빌드 지원을 추가합니다. 이 파일은 자동 병합되지 **않으므로** `-f` 플래그를 사용하여 명시적으로 전달해야 합니다. 이는 `docker compose up -d`(또는 `podman-compose up -d`)가 리포지토리를 클론했는지 여부와 관계없이 항상 GHCR에서 사전 빌드된 이미지를 가져온다는 것을 의미합니다.
...
### 1단계: `deps` (안정적 기반, ~4.5 GB)

이 레이어들은 버전 ARG가 변경되거나 APT 패키지가 변경될 때만 재빌드됩니다. 일반적인 도구만 변경되는 경우 BuildKit은 이 전체 단계를 건너뜁니다.
...
| 4. Go | 공식 tarball (최신 안정 버전, 빌드 시 결정) |
| 5. Rust | rustup (시스템 전체, 최신 안정 버전) |
| 6. Maven + Gradle | `/opt`으로 바이너리 다운로드 |
...
| 13. Playwright 브라우저 | Chromium + `playwright install --with-deps`를 통한 시스템 종속성 |
| 13b. Chrome DevTools MCP | Chrome 심볼릭 링크 + MCP 사전 캐시 + 헤드리스 패치 |
| 14. Homebrew | Linuxbrew (AI 어시스턴트 종속성 + 포매터) |
...
| 18. 엔트리포인트 | 컨테이너 시작 스크립트 (최종 COPY) |

## 도구 추가

`Dockerfile`을 편집하고 해당 섹션에 도구를 추가합니다. 추가 후 재빌드합니다:
...
섹션 10에 추가하거나 아키텍처 감지를 포함하는 새로운 `RUN` 레이어를 생성합니다:
...
CI는 **레지스트리 기반 캐싱**을 사용합니다 — 빌드 레이어가 GitHub Actions 캐시 대신 GHCR(`ghcr.io/f5xc-salesdemos/devcontainer:cache-*`)에 저장됩니다. 이는 약 6 GB 크기의 멀티 아키텍처 이미지에서 빈번한 전체 재빌드를 유발했던 10 GB GHA 캐시 제한을 우회합니다.

2단계 아키텍처는 레지스트리 캐시와 함께 다음과 같이 동작합니다:
