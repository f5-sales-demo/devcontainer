...
description: >-
  Demuestra que Xvfb, Chromium en modo con interfaz gráfica (a través de
  Playwright) y la grabación de pantalla con ffmpeg funcionan juntos
  correctamente dentro del devcontainer.
sidebar:
  order: 1
i18n:
...
Esta prueba verifica la pila completa de VNC/navegador grabando una sesión de navegador con interfaz gráfica en un archivo MP4. Demuestra que:

- **Xvfb** está en ejecución y aceptando conexiones en `:99`
- **Chromium** puede iniciarse en modo con interfaz gráfica en la pantalla virtual
...
2. Inicia la grabación de ffmpeg capturando la pantalla a 10 fps
3. Lanza Chromium con interfaz gráfica mediante la `sync_api` de Playwright
...
| `ffmpeg: x11grab error` | Discrepancia en la pantalla o ffmpeg no tiene soporte para x11grab |
