...
description: >-
  พิสูจน์ว่า Xvfb, headed Chromium (ผ่าน Playwright) และการบันทึกหน้าจอด้วย
  ffmpeg ทำงานร่วมกันได้ภายใน devcontainer
sidebar:
  order: 1
i18n:
...
การทดสอบนี้ตรวจสอบ VNC/browser stack ทั้งหมดโดยการบันทึกเซสชันเบราว์เซอร์แบบ headed เป็นไฟล์ MP4 โดยพิสูจน์ว่า:
...
- **MP4** ที่ได้เป็นวิดีโอที่ถูกต้องและเล่นได้

## วิธีการทำงาน

1. สำรวจ `xdpyinfo -display :99` จนกว่าจอแสดงผล X จะพร้อม (รูปแบบเดียวกับ entrypoint)
2. เริ่มบันทึก ffmpeg ที่ 10 fps
3. เปิด headed Chromium ผ่าน `sync_api` ของ Playwright
4. นำทางไปยัง `https://example.com` (หยุด 3 วินาที) จากนั้น `https://httpbin.org` (หยุด 3 วินาที)
5. ส่ง SIGTERM ไปยัง ffmpeg เพื่อจบไฟล์ MP4 อย่างสมบูรณ์
6. รายงานเส้นทางผลลัพธ์และขนาดไฟล์

## สคริปต์

บันทึกไฟล์ต่อไปนี้เป็น `vnc-browser-test.py` ภายในคอนเทนเนอร์ หรือคัดลอกจากไดเรกทอรี `docs/tests/` ของ repo นี้:
...
<Tabs>
<TabItem label="ภายในคอนเทนเนอร์ที่กำลังทำงาน">
...
<TabItem label="รันครั้งเดียวด้วย podman">
...
ไฟล์ MP4 ควรแสดงหน้าต่าง Chromium ที่เรนเดอร์เว็บไซต์ทั้งสองบนเดสก์ท็อป fluxbox เล่นกลับด้วย `ffplay`, `vlc` หรือดาวน์โหลดจากคอนเทนเนอร์เพื่อตรวจสอบ

## ความหมายเมื่อเกิดความล้มเหลว

| อาการ | สาเหตุที่น่าจะเป็น |
| ---- | ---- |
| `Xvfb display :99 not ready` | Xvfb ไม่ได้เริ่มต้น — ตรวจสอบว่า entrypoint ได้รัน VNC stack แล้ว |
| `ffmpeg: x11grab error` | จอแสดงผลไม่ตรงกันหรือ ffmpeg ไม่รองรับ x11grab |
| `Playwright launch error` | Chromium ไม่ได้ติดตั้งหรือขาด `--no-sandbox` |
| MP4 มีขนาด 0 ไบต์ | ffmpeg ถูกหยุดก่อนที่จะเขียนเสร็จ — ตรวจสอบการจัดการ SIGTERM |
