---
title: การทดสอบการบันทึกหน้าจอ
description: >-
  พิสูจน์ว่า Xvfb, Chromium แบบ headed (ผ่าน Playwright) และการบันทึกหน้าจอด้วย
  ffmpeg ทำงานร่วมกันได้อย่างถูกต้องภายใน devcontainer
sidebar:
  order: 1
i18n:
...
การทดสอบนี้ตรวจสอบการทำงานของ VNC/browser stack ทั้งหมด โดยการบันทึกเซสชันเบราว์เซอร์แบบ headed เป็นไฟล์ MP4 เพื่อพิสูจน์ว่า:

- **Xvfb** กำลังทำงานและรับการเชื่อมต่อบน `:99`
- **Chromium** สามารถเปิดในโหมด headed บน virtual display ได้
- **ffmpeg** สามารถจับภาพหน้าจอผ่าน x11grab ได้
- ไฟล์ **MP4** ที่ได้เป็นวิดีโอที่ถูกต้องและเล่นได้

## วิธีการทำงาน

1. ตรวจสอบ `xdpyinfo -display :99` ซ้ำจนกว่า X display จะพร้อม (รูปแบบเดียวกับ entrypoint)
2. เริ่มต้น ffmpeg บันทึกหน้าจอที่ 10 fps
...
5. ส่ง SIGTERM ไปยัง ffmpeg เพื่อสรุปไฟล์ MP4 อย่างถูกต้อง
...
ไฟล์ MP4 ควรแสดงหน้าต่าง Chromium ที่กำลังแสดงผลทั้งสองเว็บไซต์บนเดสก์ท็อป fluxbox เล่นไฟล์กลับด้วย `ffplay`, `vlc` หรือดาวน์โหลดจากคอนเทนเนอร์เพื่อตรวจสอบ

## ความหมายเมื่อทดสอบล้มเหลว

| อาการ | สาเหตุที่เป็นไปได้ |
| ---- | ---- |
| `Xvfb display :99 not ready` | Xvfb ไม่ได้เริ่มต้น — ตรวจสอบว่า entrypoint ได้รัน VNC stack แล้ว |
| `ffmpeg: x11grab error` | Display ไม่ตรงกันหรือ ffmpeg ไม่รองรับ x11grab |
| `Playwright launch error` | Chromium ไม่ได้ติดตั้งหรือขาด `--no-sandbox` |
| MP4 มีขนาด 0 ไบต์ | ffmpeg ถูกหยุดก่อนเขียนข้อมูล — ตรวจสอบการจัดการ SIGTERM |
