...
## ความหมายเมื่อการทดสอบล้มเหลว

| อาการ | สาเหตุที่เป็นไปได้ |
| ---- | ---- |
| `Xvfb display :99 not ready` | Xvfb ไม่ได้เริ่มทำงาน — ตรวจสอบว่า entrypoint ได้รัน VNC stack แล้ว |
| `ffmpeg: x11grab error` | Display ไม่ตรงกัน หรือ ffmpeg ไม่รองรับ x11grab |
| `Playwright launch error` | ไม่ได้ติดตั้ง Chromium หรือขาด `--no-sandbox` |
| MP4 มีขนาด 0 ไบต์ | ffmpeg ถูก kill ก่อนเขียนข้อมูล — ตรวจสอบการจัดการ SIGTERM |
