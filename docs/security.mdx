...
- **Capability dropping** — All Linux capabilities are dropped (`cap_drop: ALL`), then only the minimum set is re-added: `CHOWN`, `DAC_OVERRIDE`, `FOWNER` (for volume permission fixes in the entrypoint), `SETUID`/`SETGID` (for sudo), and `NET_RAW` (for ping/tcpdump).
