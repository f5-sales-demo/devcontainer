...
description: Problemas comuns e como corrigi-los.
sidebar:
  order: 8
i18n:
...
A imagem pode estar desatualizada. Baixe a mais recente e reinicie:
...
O entrypoint popula `~/.claude.json` com flags de onboarding. Se estiver faltando:
...
- **Chave do proxy LiteLLM ausente ou inválida** — se você está usando o modo proxy, certifique-se de que `LITELLM_API_KEY` no `.env` contém a credencial correta do proxy, depois reinicie o contêiner para que o entrypoint possa derivar `ANTHROPIC_API_KEY`.
- **Token OAuth ou incompatibilidade de modo proxy** — use exatamente um modo de autenticação. Se `CLAUDE_CODE_OAUTH_TOKEN` estiver definido, ele tem precedência sobre o modo proxy do LiteLLM.

### Requisições à API falham com 401

Verifique se sua chave de API está definida corretamente:

```bash
echo "$ANTHROPIC_API_KEY"
```

Confirme que ela funciona fazendo uma chamada direta ao seu provedor.
...
### Clone via HTTPS falha com 401

Quando `GH_TOKEN` está definido, o entrypoint executa `gh auth setup-git` para configurar o auxiliar de credenciais do git para HTTPS. Se as operações HTTPS falharem:
...
3. Reinicie o contêiner para re-executar o entrypoint
...
Se ambos estiverem configurados, o git usa o protocolo que corresponder à URL do remote. Para alternar um clone existente:
...
Depois reinicie o Podman: `podman machine stop && podman machine start`.
...
Se não estiver em execução, verifique se `ENABLE_VNC` está como `true` (o padrão) e confira os logs do contêiner:
...
Se o link simbólico estiver faltando, baixe a imagem mais recente e reinicie o contêiner.

### Erro de bloqueio do perfil do navegador

Com a arquitetura de navegador compartilhado, erros de bloqueio de perfil não devem ocorrer. Se você encontrar um, um processo Chrome obsoleto pode estar mantendo o bloqueio:

```bash
# Encerrar quaisquer processos Chrome obsoletos
...
O servidor chrome-devtools-mcp agora está registrado globalmente em `~/.claude/settings.json`. Se você tiver um `.mcp.json` por repositório que também o registra, verá ferramentas duplicadas. Remova a entrada `chrome-devtools-mcp` dos arquivos `.mcp.json` por repositório:
...
Isso destrói todos os dados nos volumes. Você precisará re-clonar repositórios e reconfigurar ferramentas.
