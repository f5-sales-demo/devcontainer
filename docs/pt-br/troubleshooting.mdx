...
O entrypoint inicializa `~/.claude.json` com flags de onboarding. Se estiver ausente:
...
- **Chave do proxy LiteLLM ausente ou inválida** — se você está usando o modo proxy, certifique-se de que `LITELLM_API_KEY` no `.env` contém a credencial correta do proxy, então reinicie o contêiner para que o entrypoint possa derivar `ANTHROPIC_API_KEY`.
...
O modelo é determinado pelo seu provedor e chave de API. Consulte a documentação do seu provedor para verificar os modelos disponíveis.
...
### Clone git via HTTPS falha com 401

Quando `GH_TOKEN` está definido, o entrypoint executa `gh auth setup-git` para configurar o helper de credenciais do git para HTTPS. Se as operações HTTPS falharem:

1. Verifique se o token é válido: `gh auth status`
2. Verifique se o helper de credenciais está configurado: `git config --global credential.helper`
...
Ou adicione as configurações de DNS em `~/.config/containers/containers.conf` no host:
...
O servidor chrome-devtools-mcp agora está registrado globalmente em `~/.claude/settings.json`. Se você tem um `.mcp.json` por repositório que também o registra, verá ferramentas duplicadas. Remova a entrada `chrome-devtools-mcp` dos arquivos `.mcp.json` por repositório:
