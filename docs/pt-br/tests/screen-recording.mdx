...
description: >-
  Comprova que o Xvfb, o Chromium em modo headed (via Playwright) e a gravação
  de tela com ffmpeg funcionam juntos dentro do devcontainer.
sidebar:
  order: 1
i18n:
...
Este teste verifica a pilha completa de VNC/navegador ao gravar uma sessão de navegador em modo headed para um arquivo MP4. Ele comprova que:

- O **Xvfb** está em execução e aceitando conexões em `:99`
- O **Chromium** pode ser iniciado em modo headed no display virtual
- O **ffmpeg** pode capturar o display via x11grab
- O **MP4** resultante é um vídeo válido e reproduzível

## Como Funciona

1. Consulta repetidamente `xdpyinfo -display :99` até que o display X esteja pronto (mesmo padrão do entrypoint)
2. Inicia a gravação do ffmpeg capturando o display a 10 fps
3. Lança o Chromium em modo headed via `sync_api` do Playwright
4. Navega para `https://example.com` (pausa de 3 s), depois para `https://httpbin.org` (pausa de 3 s)
5. Envia SIGTERM para o ffmpeg para finalização limpa do MP4
...
O MP4 deve mostrar a janela do Chromium renderizando ambos os sites no desktop do fluxbox. Reproduza-o com `ffplay`, `vlc`, ou faça o download a partir do container para verificar.

## O Que uma Falha Significa

| Sintoma | Causa provável |
| ---- | ---- |
| `Xvfb display :99 not ready` | O Xvfb não foi iniciado — verifique se o entrypoint executou a pilha VNC |
...
| MP4 com 0 bytes | O ffmpeg foi encerrado antes de gravar — verifique o tratamento de SIGTERM |
