...
Este teste verifica a stack completa de VNC/navegador gravando uma sessão de navegador em modo headed em um arquivo MP4. Ele comprova que:
...
3. Inicia o Chromium em modo headed via `sync_api` do Playwright
...
O MP4 deve mostrar a janela do Chromium renderizando ambos os sites na área de trabalho do fluxbox. Reproduza-o com `ffplay`, `vlc`, ou faça o download do container para verificar.

## O que uma Falha Significa

| Sintoma | Causa provável |
| ---- | ---- |
| `Xvfb display :99 not ready` | Xvfb não iniciado — verifique se o entrypoint executou a stack VNC |
...
| MP4 com 0 bytes | ffmpeg foi encerrado antes de gravar — verifique o tratamento de SIGTERM |
