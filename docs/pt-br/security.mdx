...
Muitas equipes de segurança corporativa agora exigem que ferramentas de codificação com IA sejam executadas apenas dentro de máquinas virtuais dedicadas, contêineres ou ambientes descartáveis — não diretamente nos endpoints corporativos.
...
|                                                |
|  - Nenhum binário de ferramenta de IA          |
|    instalado                                   |
...
Todos os dados residem em volumes nomeados do Docker — não em diretórios do host montados via bind. Um comprometimento dentro do contêiner **não pode** acessar arquivos na sua máquina host. Consulte [Configuração — Persistência de Dados](./configuration#data-persistence) para detalhes dos volumes.

### Sem Armazenamentos de Credenciais Compartilhados

O contêiner não tem acesso ao macOS Keychain, Windows Credential Manager, perfis de navegador ou ao agente SSH do host. As chaves SSH são injetadas na inicialização por meio de uma variável de ambiente codificada em base64 e existem apenas dentro do contêiner.

### Sem Sincronização de Área de Transferência

Contêineres Docker não compartilham o estado da área de transferência com o host. Dados não podem vazar através de copiar/colar.
...
- **Limites de recursos** — CPU (4 cores), memória (8 GB) e contagem de PIDs (4096) são limitados via `mem_limit`, `cpus` e `pids_limit` para prevenir ataques de exaustão de recursos.
- **tmpfs para `/tmp`** — O diretório `/tmp` é montado como tmpfs de 256 MB, mantendo arquivos temporários em memória e impedindo que persistam na camada gravável do contêiner.
...
- **Umask para chave SSH** — As chaves privadas SSH são escritas com `umask 077`, garantindo que o arquivo nunca fique legível para todos — mesmo brevemente — entre a criação e a chamada `chmod`.

### Ferramentas de Segurança e Teste de Penetração

O contêiner inclui aproximadamente 80 ferramentas pré-instaladas de segurança e teste de penetração para cenários de teste autorizados. As ferramentas são organizadas por categoria:
...
3. **Rotacione as chaves SSH** usadas dentro do contêiner periodicamente.
