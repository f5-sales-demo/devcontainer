...
Esta página é destinada a desenvolvedores que desejam clonar o repositório, compilar a imagem localmente ou personalizar o Dockerfile. Se você apenas deseja usar o contêiner pré-compilado, consulte [Primeiros Passos](./getting-started).
...
O arquivo `docker-compose.build.yml` adiciona suporte a build local. Ele **não** é mesclado automaticamente — você deve passá-lo explicitamente com flags `-f`. Isso significa que `docker compose up -d` (ou `podman-compose up -d`) sempre puxa a imagem pré-compilada do GHCR, independentemente de você ter clonado o repositório ou não.
...
Compare com a configuração de build explícita:
...
Isso mescla o arquivo de build com o arquivo compose base, compila a imagem a partir do `Dockerfile` local e inicia o contêiner. A flag `--build` força uma recompilação mesmo que exista uma imagem em cache.

## Arquitetura de Build em Dois Estágios

O Dockerfile usa um build em dois estágios otimizado para cache de camadas Docker:

### Estágio 1: `deps` (fundações estáveis, ~4,5 GB)

Essas camadas só são recompiladas quando um ARG de versão é atualizado ou os pacotes APT mudam. Em alterações apenas de ferramentas, o BuildKit pula este estágio inteiro.

| Seção | Conteúdo |
|-------|----------|
| 1. Repositórios APT | NodeSource, deadsnakes, HashiCorp, GitHub, Docker, Microsoft, Google Cloud, Dart SDK |
| 2. Pacotes APT | Ferramentas do sistema, Node.js, Python, Java, Terraform, GitHub CLI, Docker engine, Azure CLI, PowerShell, locales |
...
| 7. Stack VNC | Xvfb, x11vnc, noVNC, fluxbox — display remoto via navegador |
...
| 14. Homebrew | Linuxbrew (dependências de assistentes de IA + formatadores) |
| 15. Plugins ZSH | Plugins customizados oh-my-zsh |
...
Ou no VS Code: **Dev Containers &rarr; Reconstruir Contêiner**.
...
O CI usa **cache baseado em registry** — as camadas de build são armazenadas no GHCR (`ghcr.io/f5xc-salesdemos/devcontainer:cache-*`) em vez do cache do GitHub Actions. Isso evita o limite de 10 GB do cache GHA que causava recompilações completas frequentes para esta imagem multi-arquitetura de ~6 GB.

A arquitetura em dois estágios funciona com o cache de registry de forma que:
...
- **Alterações na fundação** (pacotes APT, versões Go/Rust/Java) — recompilação completa de ambos os estágios (~30 min)
