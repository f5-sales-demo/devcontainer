...
O VS Code detecta o arquivo `.devcontainer/devcontainer.json` e exibe uma notificação: **"Reopen in Container"**. Clique nela, ou use a paleta de comandos:
...
Para adicionar extensões, edite o array `customizations.vscode.extensions` no arquivo `.devcontainer/devcontainer.json`.
...
Isso reconstrói a imagem e reinicia o container. Volumes nomeados (`workspace`, `home`) persistem entre reconstruções.
