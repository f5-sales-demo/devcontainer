...
- **LiteLLMプロキシキーが未設定または無効** — プロキシモードを使用している場合、`.env` の `LITELLM_API_KEY` に正しいプロキシ認証情報が含まれていることを確認し、コンテナを再起動してエントリポイントが `ANTHROPIC_API_KEY` を導出できるようにしてください。
...
[github.com/settings/tokens](https://github.com/settings/tokens) でトークンを作成してください。きめ細かいトークン（推奨）または `repo` スコープ付きのクラシックトークンのどちらも使用できます。`.env` を更新した後、コンテナを再起動してください。
...
実行されていない場合、`ENABLE_VNC` が `true`（デフォルト）であることを確認し、コンテナログを確認してください：
