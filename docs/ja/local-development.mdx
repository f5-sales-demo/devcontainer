...
CIは**レジストリベースのキャッシュ**を使用します — ビルドレイヤーはGitHub Actionsキャッシュではなく、GHCR（`ghcr.io/f5xc-salesdemos/devcontainer:cache-*`）に保存されます。これにより、この約6 GBのマルチアーキテクチャイメージで頻繁なフルリビルドを引き起こしていた10 GBのGHAキャッシュ制限を回避できます。
