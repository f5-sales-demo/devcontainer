---
title: ローカル開発
description: リポジトリをクローンし、ローカルでビルドし、Dockerイメージをカスタマイズします。
sidebar:
  order: 5
i18n:
...
このページは、リポジトリをクローンし、ローカルでイメージをビルドしたり、Dockerfileをカスタマイズしたい開発者向けです。ビルド済みコンテナを使用するだけの場合は、[はじめに](./getting-started)をご覧ください。
...
`docker-compose.build.yml` ファイルはローカルビルドのサポートを追加します。このファイルは自動マージ**されません** — `-f` フラグを使って明示的に指定する必要があります。これにより、リポジトリをクローンしたかどうかに関わらず、`docker compose up -d`（または `podman-compose up -d`）は常にGHCRからビルド済みイメージをプルします。

<Tabs syncKey="runtime">
<TabItem label="Docker">

通常の `docker compose up` がビルド済みイメージのみを使用する（`build:` コンテキストなし）ことを確認できます：

```bash
docker compose config
```

明示的なビルド設定と比較してみてください：
...
通常の `podman-compose up` がビルド済みイメージのみを使用する（`build:` コンテキストなし）ことを確認できます：
...
これにより、ビルドファイルがベースのcomposeファイルとマージされ、ローカルの `Dockerfile` からイメージがビルドされ、コンテナが起動します。`--build` フラグは、キャッシュされたイメージが存在する場合でも強制的に再ビルドします。

## 2段階ビルドアーキテクチャ

DockerfileはDockerレイヤーキャッシュに最適化された2段階ビルドを使用しています：

### ステージ1: `deps`（安定した基盤、約4.5 GB）

これらのレイヤーは、バージョンARGが更新されるかAPTパッケージが変更された場合にのみ再ビルドされます。通常のツール変更のみの場合、BuildKitはこのステージ全体をスキップします。
...
### ステージ2: `final`（変更頻度の高いツール + ユーザー設定、約1.5 GB）

これらのレイヤーはより頻繁に変更されます（npm/pipの更新、設定変更）が、重い `deps` ステージがキャッシュされているため、素早く再ビルドされます。
...
| 12c. Rubyリンター | rubocop + 拡張 |
| 12e. Perlリンターモジュール | Perl::Critic拡張 |
| 12f. Luaリンター | luacheck |
| 12g. Rリンター | lintr |
| 12h. セキュリティRuby gems | wpscan、evil-winrm |
| 12i. Git-cloneセキュリティツール | testssl.sh、exploitdb（searchsploit）、SecLists、docker-bench-security、recon-ng、spiderfoot |
...
| 18. エントリーポイント | コンテナ起動スクリプト（最後のCOPY） |

## ツールの追加

`Dockerfile` を編集し、適切なセクションにツールを追加します。追加後に再ビルドしてください：
...
セクション10に追加するか、アーキテクチャ検出付きの新しい `RUN` レイヤーを作成します：
...
CIは**レジストリベースのキャッシュ**を使用しています — ビルドレイヤーはGitHub Actionsキャッシュではなく、GHCR（`ghcr.io/f5xc-salesdemos/devcontainer:cache-*`）に保存されます。これにより、この約6 GBのマルチアーキテクチャイメージで頻繁なフルリビルドを引き起こしていた10 GBのGHAキャッシュ制限を回避できます。

2段階アーキテクチャはレジストリキャッシュと連携して、以下のように動作します：

- **ツールバージョンの更新**（npm、pip、バイナリツール）— `final` ステージのみ再ビルド（約5分）
- **設定ファイルの変更**（`claude-config/`、`entrypoint.sh`）— 最後の数レイヤーのみ再ビルド（約1分）
