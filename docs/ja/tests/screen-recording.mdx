...
description: >-
  Xvfb、ヘッド付きChromium（Playwright経由）、およびffmpegスクリーン録画がdevcontainer内で連携して動作することを証明します。
sidebar:
  order: 1
i18n:
...
このテストは、ヘッド付きブラウザセッションをMP4ファイルに録画することで、完全なVNC/ブラウザスタックを検証します。以下を証明します：

- **Xvfb** が起動し、`:99` で接続を受け付けていること
- **Chromium** が仮想ディスプレイ上でヘッド付きモードで起動できること
- **ffmpeg** がx11grabを介してディスプレイをキャプチャできること
...
3. Playwrightの `sync_api` を介してヘッド付きChromiumを起動します
4. `https://example.com`（3秒間の一時停止）、次に `https://httpbin.org`（3秒間の一時停止）に移動します
...
MP4には、fluxboxデスクトップ上で両方のウェブサイトをレンダリングするChromiumウィンドウが表示されるはずです。`ffplay`、`vlc` で再生するか、コンテナからダウンロードして確認してください。
