...
`docker-compose.build.yml` 文件添加了本地构建支持。它**不会**自动合并——您必须通过 `-f` 标志显式传递。这意味着无论您是否克隆了仓库，`docker compose up -d`（或 `podman-compose up -d`）始终会从 GHCR 拉取预构建的镜像。
...
此命令将构建文件与基础 compose 文件合并，从本地 `Dockerfile` 构建镜像并启动容器。`--build` 标志会强制重新构建，即使已存在缓存镜像。
...
这些层仅在版本 ARG 更新或 APT 包变更时才会重新构建。在典型的仅工具变更时，BuildKit 会完全跳过此阶段。
...
| 6. Maven + Gradle | 下载二进制文件到 `/opt` |
| 7. VNC 栈 | Xvfb、x11vnc、noVNC、fluxbox——通过浏览器远程显示 |
| 8. Nerd Fonts | JetBrainsMono、Hack、FiraCode（最新发布版） |

### 阶段 2：`final`（易变工具 + 用户配置，约 1.5 GB）

这些层变更更频繁（npm/pip 更新、配置变更），但由于重量级的 `deps` 阶段已缓存，重建速度很快。
...
编辑 `Dockerfile` 并将您的工具添加到相应部分。添加后重新构建：
...
CI 使用**基于注册表的缓存**——构建层存储在 GHCR（`ghcr.io/f5xc-salesdemos/devcontainer:cache-*`）中，而非 GitHub Actions 缓存。这避免了 10 GB GHA 缓存限制导致的频繁完整重建问题（该镜像为约 6 GB 的多架构镜像）。
