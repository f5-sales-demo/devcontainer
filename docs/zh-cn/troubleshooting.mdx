...
入口点脚本会在 `~/.claude.json` 中设置引导标志。如果该文件缺失：
...
- **LiteLLM 代理密钥缺失或无效** — 如果您使用的是代理模式，请确保 `.env` 中的 `LITELLM_API_KEY` 包含正确的代理凭据，然后重启容器以便入口点脚本派生 `ANTHROPIC_API_KEY`。
...
在 [github.com/settings/tokens](https://github.com/settings/tokens) 创建令牌。细粒度令牌（推荐）或具有 `repo` 范围的经典令牌均可使用。更新 `.env` 后重启容器。

### HTTPS git clone 返回 401 失败

当设置了 `GH_TOKEN` 时，入口点脚本会运行 `gh auth setup-git` 来配置 HTTPS 的 git 凭据助手。如果 HTTPS 操作失败：

1. 验证令牌是否有效：`gh auth status`
2. 检查凭据助手是否已配置：`git config --global credential.helper`
3. 重启容器以重新运行入口点脚本
...
如果两者都已配置，git 会使用与远程 URL 匹配的协议。要切换现有克隆的协议：
...
入口点脚本会自动在 `~/.npm-global` 配置用户可写的 npm 全局前缀，因此运行时的 `npm install -g` 命令应无需 `sudo` 即可工作。如果仍然出现 EACCES 错误，请手动重新应用修复：
...
如果 DNS 解析失败，请在宿主机的 `/etc/docker/daemon.json` 中添加：
...
如果 DNS 解析失败，请在 Podman 虚拟机内配置 DNS：
...
应该为 `:99`。如果为空，请在 `.env` 或 `devcontainer.json` 中添加 `DISPLAY=:99` 并重启。
...
chrome-devtools-mcp 服务器现已在 `~/.claude/settings.json` 中全局注册。如果您有一个也注册了它的项目级 `.mcp.json`，您将看到重复的工具。请从项目级 `.mcp.json` 文件中移除 `chrome-devtools-mcp` 条目：
