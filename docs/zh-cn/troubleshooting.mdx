...
入口脚本会在 `~/.claude.json` 中写入引导标志。如果该文件缺失：
...
- **LiteLLM 代理密钥缺失或无效** — 如果您使用代理模式，请确保 `.env` 中的 `LITELLM_API_KEY` 包含正确的代理凭证，然后重启容器以便入口脚本生成 `ANTHROPIC_API_KEY`。
...
`gh` CLI 需要 `GH_TOKEN` 环境变量。将其添加到您的 `.env` 文件中：

```ini
GH_TOKEN=ghp_your-token-here
```

在 [github.com/settings/tokens](https://github.com/settings/tokens) 创建令牌。细粒度令牌（推荐）或具有 `repo` 权限的经典令牌均可使用。更新 `.env` 后请重启容器。

### HTTPS git clone 返回 401 失败

当设置了 `GH_TOKEN` 时，入口脚本会运行 `gh auth setup-git` 来配置用于 HTTPS 的 git 凭证助手。如果 HTTPS 操作失败：
...
- **HTTPS**（`.env` 中的 `GH_TOKEN`）— 使用 `https://github.com/` URL。设置更简单，无需密钥管理。

如果两者都已配置，git 会根据远程 URL 匹配的协议来选择使用哪种方式。要切换现有克隆仓库的协议：
...
入口脚本会自动在 `~/.npm-global` 配置用户可写的 npm 全局前缀，因此运行时的 `npm install -g` 命令无需 `sudo` 即可使用。如果仍然出现 EACCES 错误，请手动重新应用修复：
...
然后重启 Podman：`podman machine stop && podman machine start`。
...
如果未运行，请检查 `ENABLE_VNC` 是否被设置为 `false`，然后重启容器。

### "Cannot open display" 或 "No display" 错误

`DISPLAY` 环境变量可能未设置。请验证：

```bash
echo "$DISPLAY"
```

其值应为 `:99`。如果为空，请在您的 `.env` 或 `devcontainer.json` 中添加 `DISPLAY=:99` 并重启。
...
如果未运行，请验证 `ENABLE_VNC` 为 `true`（默认值），并检查容器日志：
...
如果符号链接缺失，请拉取最新镜像并重启容器。

### 浏览器配置文件锁定错误

使用共享浏览器架构时，不应出现配置文件锁定错误。如果出现此错误，可能是残留的 Chrome 进程持有了锁：
...
chrome-devtools-mcp 服务器现已在 `~/.claude/settings.json` 中全局注册。如果您的项目级 `.mcp.json` 也注册了它，您将看到重复的工具。请从项目级 `.mcp.json` 文件中删除 `chrome-devtools-mcp` 条目：
...
从任何项目级 `.mcp.json` 中删除 `chrome-devtools-mcp` 键以使用全局注册。
