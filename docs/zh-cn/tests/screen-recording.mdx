---
title: 屏幕录制测试
description: 验证 Xvfb、有头模式 Chromium（通过 Playwright）和 ffmpeg 屏幕录制在 devcontainer 内能够协同工作。
sidebar:
  order: 1
i18n:
...
此测试通过将有头模式浏览器会话录制为 MP4 文件来验证完整的 VNC/浏览器堆栈。它证明了：
...
1. 轮询 `xdpyinfo -display :99` 直到 X 显示器就绪（与入口点脚本相同的模式）
...
将以下内容保存为容器内的 `vnc-browser-test.py`，或从此仓库的 `docs/tests/` 目录中复制：
...
| 症状 | 可能原因 |
| ---- | ---- |
| `Xvfb display :99 not ready` | Xvfb 未启动——检查入口点脚本是否运行了 VNC 堆栈 |
