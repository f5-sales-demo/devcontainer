...
3. 在您的机器上运行 Docker 或 Podman（请参阅[快速入门](./getting-started#container-runtime)）。VS Code Dev Containers 也支持[将 Podman 作为后端](https://code.visualstudio.com/remote/advancedcontainers/docker-options#_podman)。
...
devcontainer 配置显式包含了构建文件，因此当您使用"在容器中重新打开"或"重新构建容器"时，VS Code 可以在本地构建镜像。
...
可以从 VS Code 的 **Ports** 面板按需转发其他端口。
...
这将重新构建镜像并重启容器。命名卷（`workspace`、`home`）在重新构建后会持久保留。
