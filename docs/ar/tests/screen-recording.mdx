---
title: اختبار تسجيل الشاشة
description: >-
  يثبت أن Xvfb ومتصفح Chromium بواجهة رسومية (عبر Playwright) وتسجيل الشاشة
  بواسطة ffmpeg تعمل جميعها معاً داخل حاوية التطوير.
sidebar:
  order: 1
i18n:
...
يتحقق هذا الاختبار من حزمة VNC/المتصفح الكاملة عن طريق تسجيل جلسة متصفح بواجهة رسومية إلى ملف MP4. يثبت أن:

- **Xvfb** يعمل ويقبل الاتصالات على `:99`
- **Chromium** يمكنه الإطلاق في الوضع الرسومي على الشاشة الافتراضية
...
3. يُطلق Chromium بواجهة رسومية عبر `sync_api` الخاصة بـ Playwright
4. ينتقل إلى `https://example.com` (توقف 3 ثوانٍ)، ثم `https://httpbin.org` (توقف 3 ثوانٍ)
5. يرسل SIGTERM إلى ffmpeg لإنهاء ملف MP4 بشكل سليم
6. يُبلغ عن مسار الملف الناتج وحجمه

## السكريبت

احفظ الملف التالي باسم `vnc-browser-test.py` داخل الحاوية، أو انسخه من مجلد `docs/tests/` في هذا المستودع:
...
يجب أن يعرض ملف MP4 نافذة Chromium وهي تعرض كلا الموقعين على سطح مكتب fluxbox. يمكنك تشغيله باستخدام `ffplay` أو `vlc`، أو تنزيله من الحاوية للتحقق.
...
| ملف MP4 بحجم 0 بايت | تم إيقاف ffmpeg قبل الكتابة — تحقق من معالجة SIGTERM |
