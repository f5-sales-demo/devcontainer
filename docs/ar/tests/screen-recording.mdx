...
- **Chromium** يمكنه الإقلاع في الوضع المرئي على العرض الافتراضي
- **ffmpeg** يمكنه التقاط العرض عبر x11grab
- ملف **MP4** الناتج هو فيديو صالح وقابل للتشغيل

## كيف يعمل

1. يستطلع `xdpyinfo -display :99` حتى يصبح عرض X جاهزًا (نفس النمط المستخدم في نقطة الدخول)
2. يبدأ تسجيل ffmpeg للعرض بمعدل 10 إطارات في الثانية
3. يُطلق Chromium في الوضع المرئي عبر `sync_api` الخاصة بـ Playwright
