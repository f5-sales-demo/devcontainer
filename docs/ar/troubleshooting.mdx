...
يقوم نقطة الدخول بتهيئة `~/.claude.json` بعلامات الإعداد الأولي. إذا كان مفقوداً:
...
تحقق من القيمة المعيّنة فعلياً:
...
### طلبات API تفشل بخطأ 401

تحقق من أن مفتاح API الخاص بك معيّن بشكل صحيح:

```bash
echo "$ANTHROPIC_API_KEY"
```

تحقق من عمله بإجراء استدعاء مباشر لمزود الخدمة الخاص بك.
...
تتطلب واجهة سطر أوامر `gh` متغير بيئة `GH_TOKEN`. أضفه إلى ملف `.env`:

```ini
GH_TOKEN=ghp_your-token-here
```

أنشئ رمزاً في [github.com/settings/tokens](https://github.com/settings/tokens). تعمل كل من الرموز الدقيقة (موصى بها) والرموز الكلاسيكية ذات نطاق `repo`. أعد تشغيل الحاوية بعد تحديث `.env`.

### استنساخ HTTPS عبر git يفشل بخطأ 401
...
يقوم البناء الأول بتنزيل الصورة الأساسية (~1 جيجابايت). تستخدم عمليات البناء اللاحقة ذاكرة التخزين المؤقت. إذا كان بطيئاً في كل مرة:
...
**تحذير:** يؤدي هذا إلى إزالة جميع الحاويات والصور والأقراص غير المستخدمة — وليس فقط الخاصة بهذا المشروع.
...
إذا لم يكن قيد التشغيل، تحقق من أن `ENABLE_VNC` ليس معيّناً على `false` وأعد تشغيل الحاوية.

### خطأ "Cannot open display" أو "No display"

قد لا يكون متغير البيئة `DISPLAY` معيّناً. تحقق:
...
إذا لم يكن قيد التشغيل، تحقق من أن `ENABLE_VNC` معيّن على `true` (الافتراضي) وتحقق من سجلات الحاوية:
...
مع بنية المتصفح المشترك، لا ينبغي أن تحدث أخطاء قفل ملف التعريف. إذا رأيت خطأً، فقد تكون عملية Chrome قديمة تحتفظ بالقفل:
...
تم تسجيل خادم chrome-devtools-mcp عالمياً في `~/.claude/settings.json`. إذا كان لديك `.mcp.json` خاص بالمستودع يقوم أيضاً بتسجيله، فسترى أدوات مكررة. أزل إدخال `chrome-devtools-mcp` من ملفات `.mcp.json` الخاصة بالمستودع:
...
أزل مفتاح `chrome-devtools-mcp` من أي `.mcp.json` خاص بالمستودع لاستخدام التسجيل العالمي.
...
يؤدي هذا إلى تدمير جميع البيانات في الأقراص. ستحتاج إلى إعادة استنساخ المستودعات وإعادة تكوين الأدوات.
