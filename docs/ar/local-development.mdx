...
هذه الصفحة مخصصة للمطورين الذين يرغبون في استنساخ المستودع أو بناء الصورة محلياً أو تخصيص ملف Dockerfile. إذا كنت ترغب فقط في استخدام الحاوية المُنشأة مسبقاً، راجع [البدء](./getting-started).
...
يمكنك التحقق من أن أمر `podman-compose up` العادي يستخدم فقط الصورة المُنشأة مسبقاً (بدون سياق `build:`):
...
أو في VS Code: **Dev Containers &rarr; إعادة بناء الحاوية**.
