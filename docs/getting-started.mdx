...
- **AI CLI tools** — configure `ANTHROPIC_API_KEY` and optionally the proxy if you want to use Claude Code (see Step 3)
...
### Proxy mode (OpenAI-compatible providers)
...
When `OPENAI_API_KEY` is set, the container automatically starts a built-in proxy process that translates Claude Code's native Anthropic API calls to OpenAI format. `ANTHROPIC_API_KEY` can be any non-empty value — Claude Code requires it to be set, but the proxy authenticates upstream with `OPENAI_API_KEY`.

See [Configuration — AI Proxy](./configuration#ai-proxy-openai-compatible-providers) for model mapping and tuning options.
...
```bash
# Direct mode — connects to Anthropic directly
claude -p "Say hello"

# Proxy mode — verify proxy is running
