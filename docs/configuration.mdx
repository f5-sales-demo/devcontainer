...
When set, both Claude Code and xcsh authenticate directly with Anthropic using OAuth — no API key or proxy needed. This takes priority over `LITELLM_API_KEY` for xcsh configuration. Get your token from Claude Code settings or the [Anthropic console](https://console.anthropic.com/).
...
- OpenAI-compatible endpoint → `${LITELLM_BASE_URL}/openai/v1` (used by xcsh)

| Variable | Description | Example |
|----------|-------------|---------|
| `LITELLM_BASE_URL` | Domain of your LiteLLM proxy (no path suffix) | `https://f5ai.pd.f5net.com` |
...
- `~/.config/xcsh/xcsh.json` — Anthropic-native config with `claude-opus-4-6` as the default model
- `~/.local/share/xcsh/auth.json` — OAuth credentials for Anthropic

This connects xcsh directly to Anthropic with no proxy needed.

#### LiteLLM mode

When `LITELLM_API_KEY` and `LITELLM_BASE_URL` are set (and no OAuth token), the entrypoint seeds `~/.config/xcsh/xcsh.json` with a custom provider called **OpenAI Proxy** that points at the upstream endpoint.
...
In both modes, the config is only seeded when the file doesn't already exist, so you can customize it without it being overwritten on restart. To reset to defaults, delete `~/.config/xcsh/xcsh.json` (and `~/.local/share/xcsh/auth.json` for OAuth mode) and restart the container.
...
- **xcsh**: Uses built-in [Exa](https://exa.ai) web search — no API key required. Enabled via `XCSH_ENABLE_EXA=true` or `XCSH_EXPERIMENTAL=true`
