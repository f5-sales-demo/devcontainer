...
The container supports four **mutually exclusive** authentication modes (in priority order):
...
If your LiteLLM (or other) proxy already speaks the native Anthropic Messages API — including `web_search`, streaming, and tool use — set `ANTHROPIC_BASE_URL` to route Claude Code directly to it. No built-in translation proxy is started.
...
This mode takes priority over Proxy mode: when `ANTHROPIC_BASE_URL` is set to a non-localhost URL, the built-in proxy is never started — even if `OPENAI_API_KEY` is also present.
...
The entrypoint substitutes `OPENAI_BASE_URL` and `OPENAI_API_KEY` directly into the config at container start, before the Claude Code proxy rewrites the environment variables. This ensures OpenCode connects to the upstream API endpoint rather than the local translation proxy.
