...
SearXNG is a self-hosted metasearch engine that aggregates results from Google, Bing, DuckDuckGo, and others with no API key required. A custom `searxng/settings.yml` is bind-mounted into the SearXNG container with two changes required for API access:
