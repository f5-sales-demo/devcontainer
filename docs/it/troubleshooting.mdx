...
L'entrypoint inizializza `~/.claude.json` con i flag di onboarding. Se mancante:
...
- **SSH** (`SSH_PRIVATE_KEY` in `.env`) — utilizza URL `git@github.com:`. Necessario se la vostra organizzazione impone SSH.
...
L'entrypoint configura automaticamente un prefisso globale npm scrivibile dall'utente in `~/.npm-global`, quindi i comandi `npm install -g` a runtime dovrebbero funzionare senza `sudo`. Se riscontrate ancora errori EACCES, applicate la correzione manualmente:
...
**Attenzione:** Questo comando rimuove tutti i container, le immagini e i volumi inutilizzati — non solo quelli di questo progetto.
...
# Verificate il collegamento simbolico
...
Se il collegamento simbolico è mancante, scaricate l'ultima immagine e riavviate il container.

### Errore di blocco del profilo del browser

Con l'architettura del browser condiviso, gli errori di blocco del profilo non dovrebbero verificarsi. Se ne riscontrate uno, un processo Chrome residuo potrebbe trattenere il blocco:
