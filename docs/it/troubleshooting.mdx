...
- **Token OAuth o mismatch della modalità proxy** — utilizzate esattamente una modalità di autenticazione. Se `CLAUDE_CODE_OAUTH_TOKEN` è impostato, ha la precedenza sulla modalità proxy LiteLLM.
...
Create un token su [github.com/settings/tokens](https://github.com/settings/tokens). Sia i token fine-grained (consigliati) che i token classici con scope `repo` funzionano. Riavviate il container dopo aver aggiornato `.env`.

### Il clone git via HTTPS fallisce con 401

Quando `GH_TOKEN` è impostato, l'entrypoint esegue `gh auth setup-git` per configurare il credential helper di git per HTTPS. Se le operazioni HTTPS falliscono:

1. Verificate che il token sia valido: `gh auth status`
2. Controllate che il credential helper sia configurato: `git config --global credential.helper`
...
- **SSH** (`SSH_PRIVATE_KEY` in `.env`) — utilizza URL `git@github.com:`. Necessario se la vostra organizzazione richiede SSH.
- **HTTPS** (`GH_TOKEN` in `.env`) — utilizza URL `https://github.com/`. Configurazione più semplice, nessuna gestione delle chiavi.
...
**Attenzione:** Questo rimuove tutti i container, le immagini e i volumi non utilizzati — non solo quelli di questo progetto.
...
Modificate la porta dell'host in `docker-compose.yml`:
...
Con l'architettura a browser condiviso, gli errori di blocco del profilo non dovrebbero verificarsi. Se ne riscontrate uno, un processo Chrome residuo potrebbe mantenere il blocco:

```bash
# Terminate eventuali processi Chrome residui
pkill -f 'chrome.*remote-debugging-port' || true

# Rimuovete il file di blocco
...
```bash
# Controllate la configurazione per repository
cat .mcp.json
```
