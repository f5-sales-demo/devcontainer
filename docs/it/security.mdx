...
### Nessun Montaggio di Volumi Host

Tutti i dati risiedono in volumi Docker con nome — non in directory host montate tramite bind. Una compromissione all'interno del container **non può** accedere ai file sulla macchina host. Consultare [Configurazione — Persistenza dei Dati](./configuration#data-persistence) per i dettagli sui volumi.
...
- **Validazione TLS attiva per impostazione predefinita** — `NODE_TLS_REJECT_UNAUTHORIZED` non è impostato, quindi Node.js valida i certificati TLS per impostazione predefinita. Gli utenti che necessitano di accettare certificati autofirmati (ad es. per un proxy interno) possono attivarlo aggiungendo `NODE_TLS_REJECT_UNAUTHORIZED=0` al proprio file `.env`.
