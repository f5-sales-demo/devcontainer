...
A febbraio 2025, LayerX Security Research ha divulgato una vulnerabilità critica di esecuzione di codice remoto zero-click nel framework delle estensioni Claude Desktop Extensions (DXT) di Anthropic. Le estensioni vengono eseguite senza sandbox con privilegi di sistema completi, agendo come ponti di esecuzione tra il modello linguistico e il sistema operativo locale.
...
Tutti i dati risiedono in volumi Docker nominati — non in directory dell'host montate tramite bind. Una compromissione all'interno del container **non può** accedere ai file sulla macchina host. Consultare [Configurazione — Persistenza dei Dati](./configuration#data-persistence) per i dettagli sui volumi.

### Nessun Archivio di Credenziali Condiviso

Il container non ha accesso al Portachiavi macOS, a Gestione Credenziali di Windows, ai profili del browser o all'agente SSH dell'host. Le chiavi SSH vengono iniettate all'avvio tramite una variabile d'ambiente codificata in base64 ed esistono solo all'interno del container.
...
L'intero ambiente può essere distrutto e ricreato in pochi secondi. Trattare il container come usa e getta.
...
### Strumenti Solo da Riga di Comando

Questo ambiente esegue strumenti IA come **interfacce a riga di comando esclusivamente**. Nessuna applicazione desktop, nessuna estensione del browser, nessun framework DXT. La vulnerabilità che ha motivato questo modello di isolamento non si applica all'utilizzo CLI all'interno di un container.

### Hardening del Container

Il file `docker-compose.yml` applica diverse misure di difesa in profondità:

- **Rimozione delle capability** — Tutte le capability Linux vengono rimosse (`cap_drop: ALL`), poi viene ri-aggiunto solo il set minimo: `CHOWN`, `DAC_OVERRIDE`, `FOWNER` (per la correzione dei permessi dei volumi nell'entrypoint), `SETUID`/`SETGID` (per sudo), `NET_RAW` (per ping/tcpdump) e `NET_ADMIN` (per strumenti di cattura pacchetti come Wireshark e bettercap).
...
- **Validazione TLS attiva per impostazione predefinita** — `NODE_TLS_REJECT_UNAUTHORIZED` non è impostato, quindi Node.js valida i certificati TLS per impostazione predefinita. Gli utenti che necessitano di accettare certificati autofirmati (ad es. per un proxy interno) possono attivare questa opzione aggiungendo `NODE_TLS_REJECT_UNAUTHORIZED=0` al loro file `.env`.
- **Umask per chiavi SSH** — Le chiavi private SSH vengono scritte con `umask 077`, in modo che il file non sia mai leggibile da tutti — nemmeno brevemente — tra la creazione e la chiamata `chmod`.

### Strumenti di Sicurezza e Test di Penetrazione

Il container include circa 80 strumenti di sicurezza e test di penetrazione preinstallati per scenari di testing autorizzati. Gli strumenti sono organizzati per categoria:
...
Alcuni strumenti (bettercap, Metasploit) sono disponibili solo su amd64 a causa di vincoli del packaging upstream. Gli strumenti di cattura pacchetti (tshark, bettercap) richiedono la capability `NET_ADMIN`, che è inclusa nel `docker-compose.yml` predefinito.
...
| Validazione certificati TLS abilitata per impostazione predefinita | Sì |
| Servizi interni in ascolto solo su localhost | Sì |
