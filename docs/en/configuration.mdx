...
When set, Claude Code authenticates directly with Anthropic using OAuth — no API key or proxy needed. Get your token from Claude Code settings or the [Anthropic console](https://console.anthropic.com/).
...
| Variable | Description | Example |
|----------|-------------|---------|
| `LITELLM_BASE_URL` | Domain of your LiteLLM proxy (no path suffix) | `https://litellm.example.com` |
...
Every language runtime, CLI, and tool is installed directly in the `Dockerfile` at image build time. The pre-built image is published to `ghcr.io/f5-sales-demo/devcontainer:latest` on every push to `main`, so most users never need to build locally.
