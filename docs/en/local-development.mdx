...
CI uses **registry-based caching** — build layers are stored in GHCR (`ghcr.io/f5-sales-demo/devcontainer:cache-*`) instead of the GitHub Actions cache. This avoids the 10 GB GHA cache limit that caused frequent full rebuilds for this ~6 GB multi-architecture image.
