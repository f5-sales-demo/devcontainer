---
title: 本地開發
description: 複製儲存庫、本地建置，以及自訂 Docker 映像。
sidebar:
  order: 5
i18n:
...
本頁面適用於想要複製儲存庫、本地建置映像或自訂 Dockerfile 的開發者。如果您只是想使用預建容器，請參閱[快速入門](./getting-started)。
...
`docker-compose.build.yml` 檔案提供本地建置支援。它**不會**自動合併——您必須使用 `-f` 旗標明確傳入。這表示無論您是否複製了儲存庫，`docker compose up -d`（或 `podman-compose up -d`）始終會從 GHCR 拉取預建映像。
...
您可以驗證單純的 `podman-compose up` 僅使用預建映像（無 `build:` 內容）：
...
CI 使用**基於登錄檔的快取**——建置層儲存在 GHCR（`ghcr.io/f5xc-salesdemos/devcontainer:cache-*`）中，而非 GitHub Actions 快取。這避免了 10 GB GHA 快取限制，該限制曾導致此約 6 GB 多架構映像頻繁進行完整重新建置。

兩階段架構與登錄檔快取協同運作，使得：

- **工具版本更新**（npm、pip、二進位工具）——僅重新建置 `final` 階段（約 5 分鐘）
- **設定檔變更**（`claude-config/`、`entrypoint.sh`）——僅重新建置最後幾層（約 1 分鐘）
