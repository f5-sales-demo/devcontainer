...
Cela fusionne le fichier de compilation avec le fichier compose de base, compile l'image à partir du `Dockerfile` local et démarre le conteneur. Le flag `--build` force une recompilation même si une image en cache existe.
...
| 10d. Outils Java JAR | checkstyle, google-java-format (scripts wrapper) |
...
| 17. Config Claude Code + Codex | Mémoire de connaissance des outils, politique gérée, script d'auto-test, config Codex |
| 18. Entrypoint | Script de démarrage du conteneur (tout dernier COPY) |
...
Ou dans VS Code : **Dev Containers &rarr; Reconstruire le conteneur**.
...
La CI utilise un **cache basé sur le registre** — les couches de compilation sont stockées dans GHCR (`ghcr.io/f5xc-salesdemos/devcontainer:cache-*`) au lieu du cache GitHub Actions. Cela évite la limite de 10 Go du cache GHA qui provoquait des recompilations complètes fréquentes pour cette image multi-architecture d'environ 6 Go.
