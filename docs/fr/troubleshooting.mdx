...
- **Clé du proxy LiteLLM manquante ou invalide** — si vous utilisez le mode proxy, assurez-vous que `LITELLM_API_KEY` dans `.env` contient les identifiants proxy corrects, puis redémarrez le conteneur pour que le point d'entrée puisse dériver `ANTHROPIC_API_KEY`.
- **Incompatibilité entre le jeton OAuth et le mode proxy** — utilisez exactement un seul mode d'authentification. Si `CLAUDE_CODE_OAUTH_TOKEN` est défini, il a la priorité sur le mode proxy LiteLLM.
...
Si les deux sont configurés, git utilise le protocole correspondant à l'URL distante. Pour changer un clone existant :
...
Le point d'entrée configure automatiquement un préfixe global npm accessible en écriture par l'utilisateur dans `~/.npm-global`, de sorte que les commandes `npm install -g` exécutées à l'exécution devraient fonctionner sans `sudo`. Si vous rencontrez toujours des erreurs EACCES, appliquez le correctif manuellement :
...
S'il n'est pas en cours d'exécution, vérifiez que `ENABLE_VNC` n'est pas défini à `false` et redémarrez le conteneur.
...
S'il n'est pas en cours d'exécution, vérifiez que `ENABLE_VNC` est à `true` (la valeur par défaut) et consultez les journaux du conteneur :
...
Avec l'architecture de navigateur partagé, les erreurs de verrouillage de profil ne devraient pas se produire. Si vous en rencontrez une, un processus Chrome obsolète peut détenir le verrou :

```bash
# Tuer les processus Chrome obsolètes
