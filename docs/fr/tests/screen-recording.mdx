...
description: >-
  Prouve que Xvfb, Chromium en mode graphique (via Playwright) et
  l'enregistrement d'écran ffmpeg fonctionnent tous ensemble à l'intérieur du
  devcontainer.
sidebar:
  order: 1
i18n:
...
- **Chromium** peut se lancer en mode graphique sur l'affichage virtuel
- **ffmpeg** peut capturer l'affichage via x11grab
- Le fichier **MP4** résultant est une vidéo valide et lisible

## Fonctionnement

1. Interroge `xdpyinfo -display :99` jusqu'à ce que l'affichage X soit prêt (même méthode que le point d'entrée)
2. Démarre l'enregistrement ffmpeg de l'affichage à 10 fps
3. Lance Chromium en mode graphique via la `sync_api` de Playwright
...
Le fichier MP4 devrait montrer la fenêtre Chromium affichant les deux sites web sur le bureau fluxbox. Lisez-le avec `ffplay`, `vlc`, ou téléchargez-le depuis le conteneur pour vérifier.
