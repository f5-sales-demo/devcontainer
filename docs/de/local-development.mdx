...
├── docker-compose.yml          # Basis-Compose — lädt vorgefertigtes Image
...
├── docs/                       # Dokumentationswebsite (Starlight)
...
Die Datei `docker-compose.build.yml` fügt lokale Build-Unterstützung hinzu. Sie wird **nicht** automatisch zusammengeführt — Sie müssen sie explizit mit `-f`-Flags übergeben. Das bedeutet, `docker compose up -d` (oder `podman-compose up -d`) lädt immer das vorgefertigte Image von GHCR, unabhängig davon, ob Sie das Repository geklont haben oder nicht.
...
Dies führt die Build-Datei mit der Basis-Compose-Datei zusammen, baut das Image aus dem lokalen `Dockerfile` und startet den Container. Das `--build`-Flag erzwingt einen Neubau, auch wenn ein gecachtes Image existiert.
...
Diese Layer werden nur neu gebaut, wenn ein Versions-ARG aktualisiert wird oder sich APT-Pakete ändern. Bei typischen Tool-Änderungen überspringt BuildKit diese gesamte Stufe.
...
| 13. Playwright-Browser | Chromium + Systemabhängigkeiten via `playwright install --with-deps` |
...
Bearbeiten Sie das `Dockerfile` und fügen Sie Ihr Tool zum entsprechenden Abschnitt hinzu. Nach dem Hinzufügen neu bauen:
...
Oder in VS Code: **Dev Containers &rarr; Container neu bauen**.

### APT-Pakete

Zum `apt-get install`-Block in Abschnitt 2 hinzufügen:
...
Zum `npm install -g`-Block in Abschnitt 11 hinzufügen:
...
Zum `pip install`-Block in Abschnitt 12 hinzufügen:
...
Zu Abschnitt 10 hinzufügen oder einen neuen `RUN`-Layer mit Architekturerkennung erstellen:
...
CI verwendet **Registry-basiertes Caching** — Build-Layer werden in GHCR (`ghcr.io/f5xc-salesdemos/devcontainer:cache-*`) gespeichert, anstatt im GitHub Actions Cache. Dies vermeidet das 10-GB-GHA-Cache-Limit, das bei diesem ~6 GB großen Multi-Architektur-Image häufig zu vollständigen Neubauten führte.

Die zweistufige Architektur arbeitet mit dem Registry-Cache so zusammen, dass:

- **Tool-Versionsanhebungen** (npm, pip, binäre Tools) — nur die `final`-Stufe neu gebaut wird (~5 Min.)
- **Konfigurationsdatei-Änderungen** (`claude-config/`, `entrypoint.sh`) — nur die letzten Layer neu gebaut werden (~1 Min.)
